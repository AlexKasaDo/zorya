<?php include 'header-product.php'?>

    <!-- Specialities Section Start -->
    <section id="section-specialities" data-aos="fade-down" data-aos-duration="1000">
        <div class="container">
            <div class="row ">
                <div class="col-lg-12">
                    <h4 class= "center">Варені ковбаси</h4>
                    <h4 class="small center">Перший сорт</h4>
                </div>
            </div>
        </div>

    </section>
    <!-- Specialities Section Start -->
    <div class="voffset-131"></div>



    <section id="section-history"  data-aos="fade-in" data-aos-duration="1000">
        <div class="container-fluid">
            <div class="row">

                <div class="col-lg-6 equalHW padding0">
                    <div  class="section specialities">
                        <div class="section history">
                            <div class="history-content alignVCenter">
                                <div class="container-col">
                                    <!--<h4 class="small medium right light">Discover your taste</h4>-->
                                    <h4 class="white title">Мінська</h4>
                                    <div class="voffset-30"></div>
                                    <p>  Сировина: 75 % (свинина, мясо птиці, сало бокове, яловичина.
                                    </p>
                                    <p>
                                        Поживна (харчова) цінність 100 g (г) продукту (середні значення):
                                        білки – 10,0 g (г); жири – 14,3 g (г) насичені жири – 5,8 g (г);
                                        вуглеводи – 4,0 g (г).


                                    </p>
                                    <p>Енергетична цінність (калорійність) 100 g (г) продукту (середні
                                        значення): 774 kj (кДж) / 185 kcal (ккал).


                                    </p>

                                </div>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="col-lg-6 equalHW padding0">
                    <div class="minsk history-size">
                    </div>
                </div>
            </div>
    </section>

    <!-- Booking Section Start -->
    <section id="section-booking" class="section booking">

    </section>
    <!-- Booking Section End -->

<?php include_once'footer.php' ?>